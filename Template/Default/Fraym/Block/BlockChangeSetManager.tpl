                        ...
                        {foreach $siteMenuItems as $key => $translationMenuItem}
                            <ul class="list-group">
                                <li class="list-group-item">
                                    ...
                                    <div>
                                        <a href="{if $translationMenuItem['menuItem'].translations.first().url}{$translationMenuItem['menuItem'].translations.first().url}{else}/{/if}" target="_blank"><i class="fa fa-file"></i> {$translationMenuItem['menuItem'].translations.first().title} ({_('Language')}: {if $key == 0}{_('All')}{else}{$translationMenuItem['menuItemTranslation'].locale.name}{/if})</a>
                                        <div class="tab-content">
                                            <div id="menuItem-{$translationMenuItem['menuItem'].translations.first().id}">
                                                <ul>
                                                    ...
                                                    <li>
                                                        ...
                                                        <a data-toggle="tooltip" data-placement="right" title="{if $block->user}{$block->user->username} - {/if}{formatDateTime($block->created)}" href="#" onclick="Core.getBaseWindow().Core.Block.showBlockDialog('{$block.contentId}', '{$blockId}');return false;">{if $block.name}{$block.name}{else}{_('Block Id')}: {$blockId} ({$block.extension.name}){/if} ({$block.contentId}) - {if $block.type === 1}{_('Added')}{elseif $block.type === 2}{_('Edited')}{elseif $block.type === 3}{_('Moved')}{elseif $block.type === 0}{_('Deleted')}{/if}</a>
                                                    </li>
                                                    {/foreach}
                                                </ul>
                                            </div>
